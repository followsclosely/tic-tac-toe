 ...
 */
public class MiniMaxWithAlphaBeta extends AbstractAI {
    ...
    public MiniMaxWithAlphaBeta(int depth) {
        this.depth = depth;
    }
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                if (b.playPiece(x, y, this.shape)) {
                    int moveValue = min(b, depth - 1, Integer.MIN_VALUE, Integer.MAX_VALUE);
