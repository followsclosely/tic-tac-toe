...
public class SwingSupport {
    ...
    public static void main(String[] args) {
        new SwingSupport()
                .setArtificialIntelligence(new DummyAI())
                .run();
    }

    public SwingSupport setTurn(Piece turn) {
        this.turn = turn;
        return this;
    }
    ...
    public void run() {
        ...
        bot.initialize(COMPUTER, PLAYER);
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (turn == PLAYER) {
                    if (boardPanel.setPiece(e, PLAYER)) {
                        turn = COMPUTER;
                        new Thread(() -> computerTurn(boardPanel)).start();
                    }
                } else {
        ...
        if (turn == COMPUTER) {
            computerTurn(boardPanel);
        }
    }

    private void computerTurn(Component parent) {
        Coordinate coordinate = bot.yourTurn(board);
        if (coordinate != null) {
            board.playPiece(coordinate, bot.getShape());
            turn = PLAYER;
